    ...
    <footer className="border-t border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-5 py-14 sm:px-6 lg:px-8 text-slate-900 dark:text-white z-10 relative">
          ...
          {Object.entries(footerLinks).map(([heading, links]) => (
            <div key={heading}>
              ...
              <div className="mt-5 grid gap-3">
                {links.map((link) => (
                  <Link key={link.name} href={link.href} className="text-sm text-slate-500 dark:text-slate-400 transition hover:text-cyan-300">
                    {link.name}
                  </Link>
                ))}
              </div>
      ...
      <div className="mx-auto mt-12 flex max-w-7xl flex-col gap-4 border-t border-slate-200 dark:border-white/10 pt-6 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-5">
          <Link href="/services/contact" className="transition hover:text-cyan-300">Privacy Policy</Link>
          <Link href="/services/contact" className="transition hover:text-cyan-300">Terms of Service</Link>
        </div>
      </div>
    </footer>
  );
